...
export class AppMeeting {
  ...
  @Method()
  async getMeetengs(){
    let data = await fetch('http://localhost:8485/appointment?'+new URLSearchParams({
      type:"appointments"
    })).then(resp =>{
      return resp.json();
    }).then(data =>{
  ...
  openSearchGuest(status:boolean){
    if(status){
      this.searchElement = <app-search textViewInput="Juan Perez" textInfoInput="I searched for the patient's name please." returnData={(e)=>this.getDataSearchGuest(e)} closeComponent={(e)=>this.openSearchGuest(e)} ></app-search>
    }
  }
  openNewMeeting(){
