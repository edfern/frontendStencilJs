...
export class AppPatientsSearch {
  ...
  private URLPatient:string = 'http://localhost:8485/patient';
  ...
  async getPatient(){
    ...
    let dataFetch = await fetch(this.URLPatient+'?'+new URLSearchParams({
      type:'patientsName',
  ...
  @Method()
  async getRoomAvalible(){
    let data = await fetch(this.URLPatient)
  }
