...
export class AppPatientForm {
  ...
  async updatePatient(){
    ...
    let dataUpdate = await fetch('http://localhost:8485/patient',{
      method:'PUT',
      body:data
    }).then(resp =>{
      console.log(resp)
  ...
  render() {
    ...
    let statusP = <div>
      <p>{data.fullName} he is admitted to the hospital, if you want<strong> to discharge</strong> him, click on this button.</p>
      <i class="fas fa-long-arrow-alt-down"></i><br/>
      <button class="btn btn-outline-danger" type="buttom" disabled={this.disableInput} onClick={(e)=>this.handlerStatePatient(e)}>Discharge from the hospital</button>
    </div>

    let statusPF = <div>
      <p>This patient has been discharged,
      do you want to re-enter the patient?</p>
